...
const PaperListPage: React.FC<PaperListPageProps> = ({ authLoading, currentUser }) => {
  ...
  const buildSEODescription = () => {
    if (searchTerm) {
      return `Discover ${formatCount(totalCount)} AI and machine learning research papers related to "${searchTerm}". Find implementations, collaborate with researchers, and transform papers into production code.`;
    }

    return `Browse ${formatCount(totalCount)} cutting-edge AI and machine learning research papers. Find implementations, track progress, and collaborate with the community to bring research to production.`;
  };
